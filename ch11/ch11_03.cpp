...
//A B C 순서로 클래스를 생성하고 C는 B를, B는 A를 상속받은 경우 클래스 C 타입의 객체를 생성할때 
//C B A 순서로 멤버변수의 메모리가 잡히고, A B C 순서로 멤버변수 초기화와 생성자의 body 가 실행됨
//그래서 C 의 멤버 변수 초기화 리스트에서 B나 A 의 멤버변수를 초기화 하려는 경우 메모리조차 잡히지 않았기 때문에 에러
//생성자가 parameter 를 받을경우 기본생성자를 따로 직접 구현해주거나 parameter default value 를 설정해 주지 않으면 에러발생
//parameter 가 있는 생성자를 만들경우 기본생성자는 자동으로 구현되지 않기 때문(중요!)
