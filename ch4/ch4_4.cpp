...
auto add(double x, int y) -> double //trailing 자취를 남길 수 있음
{
	return x + (double)y;
}
...
int main()
{
	...
	//int a = 123; //123 은 정수이기 때문에 a는 정수형이 될 것이다, 한줄에 정보가 두개가 있다
	
	auto a = 123; //자료형을 상항에 따라서 스스로 추론하게 하는걸 형추론이라고 하고 auto를 사용함
